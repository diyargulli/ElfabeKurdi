...
//		panel.SetActive (false);
    ...
    void Update() {
		...
		if (plText ==DeviceId && plText != "" ) {
			...
			if (int.Parse (numberTxt.text) < freeLimit) {
				panel.SetActive (false);
			}else if (int.Parse (numberTxt.text) >= freeLimit) {
    ...
    private void LerpToPage(int aPageIndex) {
        ...
        _lerp = true;
        _currentPage = aPageIndex;
		if (int.Parse (numberTxt.text) <= 34&&int.Parse (numberTxt.text)>= 1) {//mine
			numberTxt.text = (_currentPage+1).ToString();
		}
    }
    ...
    public void OnEndDrag(PointerEventData aEventData) {
        ...
        if (Time.unscaledTime - _timeStamp < fastSwipeThresholdTime &&
			Mathf.Abs(difference*10/*edited*/) > fastSwipeThresholdDistance &&
